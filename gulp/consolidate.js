...
function buildTaskFunction(engine) {
  return function() {
    buildTemplates(engine, '.tmp/app/');
    buildTemplates(engine, '.tmp/components/');
  };
}
...
for (var i=0, l=engines.length; i < l; i++) {
  ...
  gulp.task('consolidate:' + engine[0] + ':app', buildTemplates.bind(this, engine, 'client/app/**/*.tpl.html', '.tmp/app/'));
