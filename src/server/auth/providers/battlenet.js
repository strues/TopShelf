...
router.post('/', function(req, res) {
  ...
  var battletagUrl = 'https://us.api.battle.net/account/user/battletag';
  var characterUrl = 'https://us.api.battle.net/wow/user/characters';
  ...
  // Step 1. Exchange authorization code for access token.
    ...
    request.get({url: battletagUrl, headers: headers, json: true}, function(err, response, profile) {
      ...
      if (req.headers.authorization) {
        User.findOne({
          battlenet: profile.sub
        }, function(err, existingUser) {
          if (existingUser) {
            return res.status(409).send({
              message:
              'This Battlenet account is already linked to another account.'
            });
          }
          ...
          User.findById(payload.sub, function(err, user) {
            ...
            user.displayName = user.displayName || profile.battletag;
        ...
        User.findOne({
          ...
          var user = new User();
          user.battlenet = profile.sub;
          user.displayName = profile.battletag;
          user.battletag = user.battletag || profile.battletag;
