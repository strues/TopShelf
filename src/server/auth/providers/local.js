    ...
    function(token, done) {
      ...
      User.findOne({
        ...
        if (existingUser.providers.indexOf('local') === -1) {
          return res.status(400).send({
            message: 'This account has no password. Use ' +
              existingUser.providers[0] + ' to sign in'
          });
        }
        // add reset token
...
router.get('/reset/:token', function(req, res) {
  User.findOne({
    ...
    if (!user) {
      return res.redirect('/account/password/reset/invalid');
    }
    res.redirect('/account/password/reset/' + req.params.token);
  });
});
