    ...
    request.get({ url: profileUrl, qs: params, json: true }, function(err, response, profile) {
      ...
      if (req.headers.authorization) {
        User.findOne({ xenforo: profile.id }, function(err, existingUser) {
          ...
          var payload = jwt.decode(token, config.xenforo.clientSecret);
          User.findById(payload.sub, function(err, user) {
            if (!user) {
              return res.status(400).send({ message: 'User not found' });
            }
            user.xenforo = profile.id;
            user.picture = user.picture || profile.avatar.prefix + '300x300' + profile.avatar.suffix;
        ...
        User.findOne({ xenforo: profile.id }, function(err, existingUser) {
          ...
          var user = new User();
          user.xenforo = profile.id;
          user.picture = profile.avatar.prefix + '300x300' + profile.avatar.suffix;
