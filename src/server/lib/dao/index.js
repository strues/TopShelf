 ...
 * ```let Comments = mongoose.model('Comments', CommentsSchema));
    let DAO = require('../../components/DAO');
    let collection = new DAO(Comments);```
 */
 ...
 */
DAO.prototype.find = function(req, res, next, callback) {
  let identifier = this.getIdentifer(req, res);
  if (!identifier) {
    return false;
  }
  ...
  this.collection.find(identifier).lean().exec(function(err, found) {
    ...
    if (!found) {
      return next(new errors.NotFound('Sorry, unable to find what you are looking for.'));
    }
...
DAO.prototype.search = function(req, res, next, callback) {
  let identifier = this.getIdentifer(req, res);
  if (!identifier) {
    return false;
  }
  ...
  this.collection.find({
    ...
    let modified,
      finalFound = found;
    // for(let idx = 0; idx < found.results.length; idx++) {
    //   finalFound = found.results[idx].obj;
    // }
    if (callback) {
      modified = callback(finalFound);
    }
    if (modified) {
      finalFound = modified;
    }
...
DAO.prototype.findAndPopulate = function(req, res, next, populateQuery, callback) {
  let identifier = this.getIdentifer(req, res);
  if (!identifier) {
    return false;
  }
...
DAO.prototype.findAndSort = function(req, res, next, callback, sortFilter) {
  let identifier = this.getIdentifer(req, res);
  if (!identifier) {
    return false;
  }
...
*/
DAO.prototype.findAll = function(req, res, next, callback) {
...
*/
DAO.prototype.create = function(req, res, next, callback) {
  this.collection.create(req.body, function(err, found) {
    ...
    let allFound = getArguments(arguments);
    if (callback) {
      callback(allFound);
    }
...
DAO.prototype.updateById = function(req, next, res, callback) {
  if (req.body._id) {
    delete req.body._id;
  }
  this.collection.findById(req.params.id, function(err, found) {
    ...
    let updated = _.merge(found, req.body);
...
DAO.prototype.updateOneAndUpdate = function(req, res, next, callback) {
  let self = this;
  let identifier = this.getIdentifer(req, res);
  if (!identifier) {
    return false;
...
DAO.prototype.createRaw = function(content) {
  this.collection.create(content, function(err, found) {
    if (err) {
      return console.log('createRaw error', err);
    }
    // Since mongoose returns created items as list of params we must iterate through them
    let allFound = getArguments(arguments);
  });
...
DAO.prototype.delete = function(req, res, callback, preventDeleteAll) {
  ...
  if (preventDeleteAll && utils.isEmpty(identifier)) {
...
DAO.prototype.deleteOneAndUpdate = function(req, res, next, callback) {
  let self = this;
  let identifier = this.getIdentifer(req, res);
  if (!identifier) {
    return false;
...
DAO.prototype.getIdentifer = function(req, res) {
  ...
  if (utils.containsUndefined(identifier)) {
