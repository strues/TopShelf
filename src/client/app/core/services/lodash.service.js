(function () {
    ...
    angular.module('app.core').factory('_', LodashService);
    ...
    function LodashService($window) {
        ...
        return _;
        /**
                     * Group an array of objects into several
                     * arrays by the given
                     * criteria, each group sorted by groupSort,
                     * each group consists
        ...
        function groupFilter(data, criteria, groupSort, groupItemsName) {
            ...
            return _.chain(data).sortBy(groupSort)
            .groupBy(criteria).pairs().map(function (currentItem) {
                return _.object(_.zip([
                  'key',
                  groupItemsName
                ], currentItem));
            }).sortBy('key').value();
